...
export function ApiKeys() {
  // Per-row reveal state, keyed by index.
  ...
  return (
    <div className="flex flex-col gap-[18px]">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <p className="m-0 max-w-[520px] text-[14.5px] text-muted">
          API keys authenticate your server requests. Keep secret keys server-side and rotate them
          if exposed.
        </p>
        <button
          type="button"
          className="flex cursor-pointer items-center gap-[7px] rounded-[10px] border-none bg-citron px-4 py-2.5 text-[14px] font-semibold text-ink"
        >
          <Plus size={16} />
          Create key
        </button>
      ...
      <div className="overflow-hidden rounded-2xl border border-hairline bg-card">
        ...
        {API_KEYS.map((k, i) => (
          ...
          >
            ...
            <span className="overflow-hidden text-ellipsis whitespace-nowrap font-mono text-[13px] text-text-3">
            ...
            <div className="flex justify-end gap-1.5">
              <button
                ...
                onClick={() => toggle(i)}
                title="Reveal"
                className="flex h-[30px] w-[30px] cursor-pointer items-center justify-center rounded-lg border border-hairline bg-white text-muted transition-colors hover:border-forest"
              >
